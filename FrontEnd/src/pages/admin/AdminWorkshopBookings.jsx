...
import { FiArrowLeft, FiUser, FiMail, FiCalendar, FiBook, FiSearch, FiTrash2, FiUsers } from "react-icons/fi";
...
function AdminWorkshopBookings() {
    ...
    const fetchData = async () => {
        try {
            ...
            // Fetch workshop details for title
            const workshopRes = await fetch(`${API_BASE_URL}/workshops/${id}`);
            if (workshopRes.ok) {
                const workshopData = await workshopRes.ok ? await workshopRes.json() : null;
                setWorkshop(workshopData);
            }

            // Fetch bookings
            const bookingsRes = await fetch(`${API_BASE_URL}/workshops/${id}/bookings`, {
                headers: {
                        ...
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-gray-600 dark:text-gray-400 mt-1 text-sm md:text-lg truncate">
                            ...
                            <table className="w-full">
                                <thead className="bg-gray-50 dark:bg-rustic-slate border-b border-gray-200 dark:border-rustic-ash">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 text-left text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Booking Date</th>
                                        <th className="px-6 py-4 text-center text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Tickets</th>
                                        <th className="px-6 py-4 text-left text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Status</th>
                                        <th className="px-6 py-4 text-right text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Total Paid</th>
                                    </tr>
                                </thead>
                                        ...
                                        currentItems.map((booking) => (
                                            <tr key={booking.id} className="hover:bg-gray-50 dark:hover:bg-rustic-slate transition-colors">
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-10 h-10 rounded-full bg-purple-100 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 flex items-center justify-center font-bold">
                                                            {booking.user_name.charAt(0)}
                                                        </div>
                                                        <div>
                                                            <p className="font-semibold text-gray-900 dark:text-white">{booking.user_name}</p>
                                                ...
                                                <td className="px-6 py-4 text-center text-gray-900 dark:text-white font-bold">{booking.tickets}</td>
                                                <td className="px-6 py-4">
                                                ...
                                                <td className="px-6 py-4 text-right text-gray-900 dark:text-white font-bold">LKR {booking.total_price.toLocaleString()}</td>
                                            </tr>
                                        ))
                                    ) : (
                        ...
                        {currentItems.length > 0 ? (
                            currentItems.map((booking) => (
                                <div key={booking.id} className="card-premium p-5 flex flex-col gap-4">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-full bg-purple-100 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 flex items-center justify-center font-bold text-lg">
                                            {booking.user_name.charAt(0)}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 py-4 border-y border-gray-200 dark:border-rustic-ash">
                                        <div>
                                            <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Tickets</p>
                                            <p className="text-sm text-gray-900 dark:text-white font-bold">{booking.tickets}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Total Paid</p>
                                            <p className="text-sm text-rustic-green font-bold">LKR {booking.total_price.toLocaleString()}</p>
                                        </div>
                                        <div className="col-span-2">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <p className="text-[10px] text-gray-500 font-medium uppercase">{new Date(booking.booking_date).toLocaleTimeString()}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
